...
template <>
struct Film::accessible_pour_tests_par<::testing::Test>
{
	template <typename T> // Template pour ne pas avoir une version const et non const.
	static auto &realisateur(T &&film)
	{
...
TEST(tests_ListeFilms, trouver)
{
	...
	{
		Film *film = li.trouver([&](const Film &f)
								{ return AccesFilmPourTests::realisateur(f) == r; });
...
int main(int argc, char *argv[])
{
	...
	for (int i = items.size() - 1; i >= 0; --i) // parcour le vecteur a l envers
	{
		listeItems.push_front(items[i].get());
	}
	// 1.2
	...
	// 1.3
	forward_list<Item *> copieListeItems;
	auto it = copieListeItems.before_begin();
	for (auto &item : listeItems)
	{
